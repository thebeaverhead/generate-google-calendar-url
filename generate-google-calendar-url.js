  ...
  var moment = typeof module === "object" ? require("moment") : window.moment,
    ...
    toAllDay = function(options) {
      ...
      return moment.isValid()
        ...
        "/" +
        moment.add(1, "d").format("YYYYMMDD")
        : "";
    },
    toIsoHour = function(date, ignoreTimezone = false) {
      return (
        moment(date)
          .utc()
          .format("YYYYMMDDTHHmmss") + (!ignoreTimezone ? "Z" : "")
      );
    },
    toHour = function(options) {
      ...
      const ignoreTimezone = options.ignoreTimezone ? true : false;

      return (
        "&dates=" +
        toIsoHour(options.start, ignoreTimezone) +
        "/" +
        toIsoHour(options.end, ignoreTimezone)
      );
    },
